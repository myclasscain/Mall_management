...
const mutations = {
  ...
  SET_RESULTASYNCROUTES(state, asyncRoutes) {
    // vuex保存当前用户的路由， 注意： 一个用户需要展示的路由： 常量 异步 任意路由
    ...
    // 给路由添加新的路由
    router.addRoutes(asyncRoutes.concat(anyRoutes))
  }
}
...
const actions = {
  ...
  async login({ commit }, userInfo) {
    ...
    // 注意: 当前登录请求使用的是mock的数据,mock数据的code是20000
    ...
    return new Promise((resolve, reject) => {
      ...
      getInfo(state.token)
        .then(response => {
          ...
          // }
          // 获取用户信息: 返回的数据包含: 用户name 用户头像avatar
          //  routes[返回的标志:不同的用户应该展示那些菜单 菜单权限的标记]
          // roles:用户角色的信息
